...
class TrackPlayer {
    ...
    get isMute() {
        return isMuteStore.getValue();
    }

    get lastVolume() {
        return lastVolumeStore.getValue();
    }
    ...
    private normalizeVolume(volume: number) {
        ...
        return Math.round(Math.max(0, Math.min(1, safeVolume)) * 100) / 100;
    }

    private setMuteState(isMute: boolean) {
        isMuteStore.setValue(isMute);
        setUserPreference("mute", isMute);
    }

    private setLastVolume(volume: number) {
        const v = this.normalizeVolume(volume);
        lastVolumeStore.setValue(v);
        setUserPreference("lastVolume", v);
    }
    ...
    private createAudioController() {
        ...
        audioController.onVolumeChange = (rawVolume) => {
            const volume = this.normalizeVolume(rawVolume);
            currentVolumeStore.setValue(volume);
            setUserPreference("volume", volume);
            // 防漂移：非 TrackPlayer 入口的音量变化也同步 mute/lastVolume
            if (volume > 0) {
                if (this.isMute) {
    ...
    public async setup() {
        ...
        // restore mute/lastVolume state
        const restoredLastVolume = (lastVol !== null && lastVol !== undefined)
            ? this.normalizeVolume(lastVol)
            : 0.5;
        this.setLastVolume(restoredLastVolume);

        if (mute) {
            this.setMuteState(true);
            this.audioController.setVolume(0);
            currentVolumeStore.setValue(0);
            setUserPreference("volume", 0);
        }
    ...
    public setVolume(volume: number) {
        ...
        this.audioController.setVolume(normalized);
        if (normalized > 0) {
            this.setLastVolume(normalized);
            this.setMuteState(false);
        }
    }

    public adjustVolume(delta: number) {
        if (this.isMute && delta > 0) {
            // 静音状态下增加音量：先恢复再调整
            const base = this.lastVolume > 0 ? this.lastVolume : 0;
            this.setMuteState(false);
            this.setVolume(base + delta);
        } else {
    ...
    public setMute(isMute: boolean) {
        if (isMute) {
            ...
            this.setMuteState(false);
            const restoreVolume = this.lastVolume > 0 ? this.lastVolume : 0.5;
    ...
    public async setQuality(quality: IMusic.IQualityKey) {
        ...
        try {
            ...
            if (!this.hasPlayableMediaSource(mediaSource)) {
                logger.logError(
                    "quality media source is empty",
                    new Error("quality media source is empty"),
                    {
    ...
    public async fetchCurrentLyric(forceLoad = false) {
        ...
        try {
            ...
            if (!lyricSource?.rawLrc && !lyricSource?.translation && !lyricSource?.romanization) {
    ...
    private async fetchMediaSource(musicItem: IMusic.IMusicItem, quality?: IMusic.IQualityKey) {
        ...
        let realQuality: IMusic.IQualityKey = qualityOrder[0] ?? quality ?? defaultQuality;
    ...
    private setTrack(mediaSource: IPlugin.IMediaSourceResult, musicItem: IMusic.IMusicItem, options: ITrackOptions = {
        ...
        if (!this.hasPlayableMediaSource(mediaSource)) {
            logger.logError(
                ...
                new Error("media source is empty"),
                musicItem,
            );
            throw new Error("mediaSource.url is empty");
        }
